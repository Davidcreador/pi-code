...
Local desktop development uses `d4-dev://app`, while packaged builds use `d4://app`. Add the
matching origin to each Clerk instance's Backend API `allowed_origins` array as well. The development
Clerk instance should only need `d4-dev://app`; the production Clerk instance should only need
`d4://app`. `@clerk/electron` owns the native request adapter, encrypted Clerk token persistence,
...
The production macOS bundle ID is `com.d4.desktop`. To enable native passkeys:

1. Create an explicit macOS App ID for `com.d4.desktop` in the Apple Developer portal and enable
   **Associated Domains**.
...
4. Confirm Clerk serves `https://<frontend-api>/.well-known/apple-app-site-association` and that
   `webcredentials.apps` contains `<TEAM_ID>.com.d4.desktop`.
...
T3CODE_PORT=13773 \
  "/Applications/piCode.app/Contents/MacOS/piCode"
```
...
```sh
codesign --verify --deep --strict "/Applications/piCode.app"
codesign -d --entitlements :- "/Applications/piCode.app"
```
