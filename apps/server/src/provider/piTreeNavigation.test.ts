...
describe("reconcilePiTranscript", () => {
  it.effect("dispatches only when the persisted projection drifted", () =>
    Effect.gen(function* () {
      const fixture = dependencies();
      const unchanged = yield* reconcilePiTranscript(
        input.threadId,
        { messages: [], activities: [] },
        { messages: [], activities: [] },
        fixture.value,
      ...
      const repaired = yield* reconcilePiTranscript(
        input.threadId,
        { messages: [], activities: [] },
        {
          messages: [],
...
describe("reconcilePiTranscriptPreservingPendingUserMessage", () => {
  it.effect("keeps every queued user message while replacing drifted activities", () =>
    Effect.gen(function* () {
      const fixture = dependencies();
      ...
      const repaired = yield* reconcilePiTranscriptPreservingPendingUserMessage(
        input.threadId,
        { messages: [], activities: [] },
        {
  ...
  it.effect("keeps a queued user message when transcript drift has the same length", () =>
    Effect.gen(function* () {
      const fixture = dependencies();
...
describe("navigatePiTreeAndReplaceTranscript", () => {
  it.effect("replaces the transcript only after successful navigation", () =>
    Effect.gen(function* () {
      const fixture = dependencies();
      yield* navigatePiTreeAndReplaceTranscript(input, fixture.value);
  ...
  it.effect("does not replace the transcript when navigating to the active leaf", () =>
    Effect.gen(function* () {
      const fixture = dependencies({ cancelled: false, aborted: false, leafId: "current-leaf" });
      yield* navigatePiTreeAndReplaceTranscript(
        { ...input, targetId: "current-leaf" },
        fixture.value,
      );
  ...
  ])("does not replace after cancelled or aborted navigation", (navigation) =>
    Effect.gen(function* () {
  ...
  it.effect("does not report success when replacement dispatch fails", () =>
    Effect.gen(function* () {
      const fixture = dependencies();
