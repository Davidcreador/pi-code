...
/**
 * Installs piCode as a per-user boot service. Linux-only for now: systemd
...
const BOOT_SERVICE_NAME = "d4";

export const BOOT_SERVICE_UNIT_FILE = `${BOOT_SERVICE_NAME}.service`;
export const BOOT_SERVICE_UNIT_ENV = "D4_BOOT_SERVICE_UNIT";
...
export function renderBootServiceUnit(plan: BootServicePlan): string {
  ...
  return [
    ...
    "Type=simple",
    "WorkingDirectory=%h",
    `Environment=D4_HOME=${quoteSystemdValue(plan.baseDir)}`,
...
export class BootServiceInstallError extends Schema.TaggedErrorClass<BootServiceInstallError>()(
  ...
  override get message(): string {
    return "Could not set up the piCode background service.";
  }
}
...
export const make = Effect.fn("cloud.boot_service.make")(function* (input: {
  ...
  const status: BootService["Service"]["status"] = Effect.gen(function* () {
    ...
    // references still exists (a pinned runtime under ~/.d4 can be deleted to
