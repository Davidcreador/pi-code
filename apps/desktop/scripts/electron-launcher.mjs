// This file mostly exists because we want dev mode to say "piCode (Dev)" instead of "electron"
...
const devBundleIdSuffix = NodePath.basename(repoRoot)
  .toLowerCase()
  .replaceAll(/[^a-z0-9]+/g, "");
export const APP_DISPLAY_NAME = isDevelopment ? "piCode (Dev)" : "piCode";
export const APP_BUNDLE_ID = isDevelopment
  ? `com.d4.desktop.dev.${devBundleIdSuffix || "local"}`
  : "com.d4.desktop";
const APP_PROTOCOL_SCHEMES = isDevelopment ? ["d4-dev"] : ["d4"];
...
function buildMacLauncher(electronBinaryPath) {
  ...
  if (isDevelopment) {
    ...
    // Electron.app even though this bundle's Info.plist has the piCode name.
