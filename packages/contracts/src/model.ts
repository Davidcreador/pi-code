...
/**
 * Static fallbacks only. Pi resolves its real default model itself; the
 * provider snapshot marks pi's currently selected model as default, so these
 * strings only surface when no snapshot is available yet.
 */
export const DEFAULT_MODEL = "anthropic/claude-sonnet-4-6";
export const DEFAULT_TEXT_GENERATION_MODEL = "anthropic/claude-haiku-4-5";
...
> = {
  [PI_DRIVER_KIND]: DEFAULT_TEXT_GENERATION_MODEL,
};
