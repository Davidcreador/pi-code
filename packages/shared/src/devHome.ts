 ...
 * `~/.d4` that a user's installed d4 runs against.
 *
 * A linked git worktree gets its own (gitignored) `.d4`: feature work in a
 * throwaway branch must not share a database with the real app, and an ambient
 * `D4_HOME` counts as an explicit base dir — flipping the state directory
...
export const resolveWorktreeT3Home = (
  ...
  Effect.gen(function* () {
    ...
    const path = yield* Path.Path;
    return path.join(worktreePath, ".d4");
  });
